...
export class FavoritesView extends Favorites {
  ...
  onRemove() {
    ...
    rows.forEach((row) => {
      const login = row.querySelector("td div div a").textContent
  ...
  createRow() {
    ...
    this.entries.forEach((entry) => {
      ...
      const td = `           
        <td colspan= "3">
          <div class="first-td">
            <a href="${entry.avatar}" target="_blank">
              <img
              src="${entry.avatar}"
            ...
            <div>
              <p>${entry.name}</p>
               <a
                href="https://github.com/${entry.login}" target="_blank"
                >/${entry.login}</a>
            </div>
          </div>
        </td>
        <td>
          <p>${entry.repositories}</p>
        </td>
        <td>
          <p>${entry.followers}</p>
        </td>
